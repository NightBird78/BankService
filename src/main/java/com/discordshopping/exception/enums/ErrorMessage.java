package com.discordshopping.exception.enums;

public class ErrorMessage {
    private ErrorMessage(){}
    public static final String DATA_NOT_FOUND = "data not found";
    public static final String DATA_EXIST = "data with this data already exist";
    public static final String INVALID_UUID_FORMAT = "invalid uuid format";
    public static final String INVALID_CURRENCY = "invalid currency";
    public static final String INVALID_EMAIL_FORMAT = "invalid email format";
    public static final String INVALID_IDBA = "invalid idba";
    public static final String TECHNICAL_ERROR = "technical error";
}
